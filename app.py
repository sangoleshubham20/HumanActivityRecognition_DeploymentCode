...
page_config = {"page_title": "Human Activity Recognition", "page_icon": img_logo, "layout": "centered"}
st.set_page_config(**page_config)

page = option_menu(
    menu_title=None,
    options=["Home", "Analysis", "Classification", "Code"],
    icons=["house-fill", "book", "motherboard", "file-earmark-code"],
    default_index=0,
...
if page == "Analysis":
    def analysis():
        st.text("")
        st.text("")
        st.caption('### **PIE PLOT OF ACTIVITIES :**')
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("pie_plot.png")
            st.image(img)
        st.markdown("""The pie chart represents the distribution of different activities in the dataset. Each
                    section of the pie corresponds to a specific activity, and the size of the section
                    represents the proportion of the dataset that corresponds to that activity. Let's say we
                    have **"N"** data points in our dataset, and there are **"n"** different activities in the
                    dataset. Let's also define a variable **"C_i"** to represent the number of data points that
                    correspond to the i-th activity.
                 """)
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("01.png")
            st.image(img)
        st.write("""This is because the total angle of a circle is 360 degrees, and we want to represent each
                    proportion as an angle in the circle. Therefore, the size of the i-th sector in the pie
                    chart is proportional to C_i, and the angle of the sector is proportional to p_i.
                 """)
        st.markdown("""<hr style="height:0.5px;border:none;color:#333;background-color:#333;" /> """,
                    unsafe_allow_html=True)

        st.caption('### **ANGLE BETWEEN X-AXIS & GRAVITY_MEAN WITH RESPECT TO ACTIVITIES**')
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("angleBW_xaxis.png")
            st.image(img)
        st.markdown("""This chart is a bar chart that displays the angle between X-axis and gravity_mean with
                    respect to different activities in a dataset. Each activity is represented by a different
                    color, and the length of each bar indicates the proportion of data points associated with
                    that activity that fall into a given range of angles. Mathematically, this chart represents
                    the distribution of the feature **"angle(X,gravityMean)"** across different activities. The
                    x-axis represents the values of the feature, and the y-axis represents the different
                    activities. The formula for calculating the angle between two vectors is:
                 """)
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("angle 02.png")
            st.image(img)
        st.markdown("""where **"u"** and **"v"** are the two vectors, and **"||u||"** and **"||v||"** represent
                    their magnitudes. In this chart, the angle is calculated between the X-axis and the gravity_mean
                    vector, which is likely a vector representing the orientation of a device in three-dimensional
                    space. By examining the distribution of this angle across different activities, we may gain
                    insight into how the orientation of the device varies depending on the user's activity.
                    """)
        st.markdown("""<hr style="height:0.5px;border:none;color:#333;background-color:#333;" /> """,
                    unsafe_allow_html=True)

        st.caption('### **BOXPLOT OF ACTIVITIES WITH RESPECT TO SUBJECT**')
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("boxplot.png")
            st.image(img)
        st.write("""The resulting plot will show a box plot of the different activities with respect to the
                    subjects in our dataset. The y-axis will display the different subjects, while the x-axis
                    will display the different activities. The boxes will be color-coded based on the different
                    activities, with each activity having a different color. The **boxmode=overlay** argument
                    specifies that the boxes should be overlaid on top of each other, allowing for easy
                    comparison between the different activities for each subject.
                 """)
        st.markdown("""<hr style="height:0.5px;border:none;color:#333;background-color:#333;" /> """,
                    unsafe_allow_html=True)

        st.caption('### **STATIONARY VS MOVING ACTIVITIES**')
        st.markdown("""The plot is created using a FacetGrid from the seaborn library, which allows us to create a
                    subplot for each category of the hue variable ("Activity" in this case). Then, for each
                    subplot, a distribution plot (distplot) is created for the values of the **"tBodyAccMag-mean()"**
                    feature. The two annotations in the plot indicate the regions of the distribution that
                    correspond to Stationary and Moving activities, respectively. Mathematically, the distribution
                    plot can be represented by a probability density function (PDF) that shows the probability of
                    a given value occurring in the distribution. In this case, we can write:
                 """)
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("stationary 03.png")
            st.image(img)
        st.write("""where f(x) is the PDF, and x is the value of the "tBodyAccMag-mean()" feature. The area
                    under the PDF curve between two values a and b represents the probability of the feature
                    taking a value between a and b, and is given by the integral:
                 """)
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
        ...
        plt.annotate("Stationary Activities", xy=(-0.956, 10), xytext=(-0.8, 14), size=20, va='center',
                     ha='left',
        ...
        st.caption('### **DATA PROVIDED BY EACH USER**')
        st.write("""The chart is a countplot which shows the number of data points for each activity recorded by
                    each volunteer in the dataset. In mathematical terms, we can say that the chart shows the
                    distribution of data points for different activities across volunteers.
                 """)
        col1, col2, col3 = st.columns([1, 3, 1])
        with col2:
            img = Image.open("05.png")
            st.image(img)
        st.write("""where $Count(activity_i, volunteer_j)$ is the count of data points for activity $i$ recorded
                    by volunteer $j$, $I(activity_i, volunteer_j, data_k)$ is an indicator function which is
                    equal to 1 if data point $k$ belongs to activity $i$ recorded by volunteer $j$, and $n$ is
                    the total number of data points in the dataset.
                 """)
        st.write("""The x-axis of the chart represents the volunteers, while the y-axis represents the count of
                    data points. The different activities are represented by different colors in the chart. The
                    sns.countplot() function is used to create the chart. It takes the x-axis and hue arguments
                    to specify the grouping variables, and the data argument to specify the dataset. The
                    plt.xlabel(), plt.ylabel(), and plt.title() functions are used to set the labels and title
                    of the chart, and the sns.set_style() function is used to set the style of the chart.
                 """)
        st.set_option('deprecation.showPyplotGlobalUse', False)
...
if page == "Classification":
    ...
    st.markdown("""**This Machine Learning application allows you to predict the Human Activities when given
                   Accelerometer & Gyroscope readings of a User.**
                """)
    st.text("")

    st.write("Please choose your parameters :")
    st.text("")
    ...
    if st.button('**Classify**'):
        classify = model.predict([[tBodyAccmeanX, tBodyAccmeanY, tBodyAccmeanZ, tBodyAccstdX, tBodyAccstdY,
                                   ...
                                   tBodyAccarCoeffZ2, tBodyAccarCoeffZ3, tBodyAccarCoeffZ4,
                                   tBodyAcccorrelationXY,
                                   tBodyAcccorrelationXZ, tBodyAcccorrelationYZ, tGravityAccmeanX,
                                   tGravityAccmeanY,
        ...
        with col1:
            if ans == 'LAYING':
                st.markdown(
                    "![Alt Text](https://media.giphy.com/media/v1"
                    ".Y2lkPTc5MGI3NjExZWQ0NzRlNzllOWE4N2MxYTg5MjY1NzRhYTUyMDhmZGY0MDdhYjQyZCZjdD1n"
                    "/fuXbEQRWWqJbpBusNk/giphy.gif)")
            elif ans == 'SITTING':
                ...
                st.markdown(
                    "![Alt text](https://media.giphy.com/media/v1"
                    ".Y2lkPTc5MGI3NjExNjViMDc1ZTJlYWZkZWMzMGYxOTBkZGY3YTNmNzMzOWFmYjhlZGFhNCZjdD1n"
                    "/EX1gbgvY4U3UDS7433/giphy.gif)")
...
if page == "Code":
    ...
    st.text("")
    st.caption("**DEPLOYMENT** : [link](https://github.com/sangoleshubham20/HumanActivityRecognition_DeploymentCode)")
    st.caption("**MODELLING** : [link](https://github.com/sangoleshubham20/HumanActivityRecognition_ModellingCode)")
